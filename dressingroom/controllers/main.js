    ...
    .then(function (result) {
      ...
      navPills.forEach((e) => {
        ...
        if (e.showName == "Áo") {
          ...
          getElement(".tab-content").innerHTML += `<div id="${e.type}" style="display:flex"   class="tab-pane fade active in show"></div>	`;
        } else {
          contentTab += `
		<li class="nav-item">
		<a class="nav-link" data-toggle="tab" id="${e.tabName}" href="#${e.type}" onclick=zIndex(${e.tabName}) >${e.showName}</a>
	  </li>`;
          getElement(".tab-content").innerHTML += `<div id="${e.type}" style="display: none" class="tab-pane fade"></div>	`;
        }
...
const choseItem=(id)=>{
	const item=JSON.parse(localStorage.getItem("tabPanes")).filter(i=>i.id==id.id)
	if(item[0].type=="topclothes"){
		getElement(".bikinitop").style.background="url(" + item[0].imgSrc_png + ")";
	}
	else if(item[0].type=="botclothes"){
		getElement(".bikinibottom").style.background="url(" + item[0].imgSrc_png + ")";
	}
		...
		getElement(".necklace").style.background="url(" + item[0].imgSrc_png + ")";
	}
	else{
		console.log(item[0].type)
		getElement(`.${item[0].type}`).style.background="url(" + item[0].imgSrc_png + ")";
	}
}	
